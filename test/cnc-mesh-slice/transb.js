...
function getSplit(input, output, y1, y2, bFalse = false) {
        ...
        if (bFalse) {
            if (dd.indexOf('B') !== -1) {
                ...
                for (let j = 0; j < dds.length; j++) {
                    if (dds[j].indexOf('B') !== -1) {
                        const ddsv = dds[j];
                        const value = ddsv.slice(1);
                        dds[j] = `B${-value}`;
                    }
                }
                datas[i] = dds.join(' ');
...
function range(dt, start, end, angle = 5, bFalse = false) {
    const file = `${dt + start}-${end}.cnc`;
    const file2 = `${dt + start}-${end}.2.cnc`;
    getSplit(`./input/${dt}.cnc`, `./output/${file}`, `Y${start}`, `Y${end}`, bFalse);
...
// range('t', 30, 30.1, 1);
// range('t', 29.8, 30, 1, true);
range('d', 27.9, 37.9, 1, false);
// range('d', 27.1, 27.3, 1);
