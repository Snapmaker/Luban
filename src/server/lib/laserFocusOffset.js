...
function countPixel(image) {
            ...
            if (data[id] === 0 && !visited[i][j]) {
                ...
                while (q.length > 0) {
                    ...
                    for (let k = 0; k < 4; k++) {
                        ...
                        if (i2 > 0 && i2 < width - 1 && j2 > 0 && j2 < height - 1 && data[id2] === 0 && !visited[i2][j2]) {
                            q.push({ x: i2, y: j2 });
                            visited[i2][j2] = 1;
                            count++;
                        }
    ...
    for (let i = 0; i < spots.length; i++) {
        const k = i % 5 === 0 ? Math.ceil(spots[i] / 2) : spots[i];
        if (k < minCount) {
            minIndex = i;
            minCount = k;
        }
...
export default laserFocusOffset;
