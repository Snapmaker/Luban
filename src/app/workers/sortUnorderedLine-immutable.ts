...
const sortUnorderedLine = (fragments: TPoint[], actionID: string, m: number) => {
    const m3 = new Date().getTime();
    console.log(`[${actionID}] worker exec 2, time=`, m3 - m);
    ...
    return new Promise((resolve) => {
        try {
            ...
            while (pointsMap.size) {
                ...
                if (isCircle) {
                    const arr = polygon.toJS();
                    const leng = arr.length;
                    const arr2 = [];
                    for (let index = 0; index < leng; index++) {
                        const next = arr[index + 1] || arr[0];
                        arr2.push(arr[index], next);
                    }
                    polygons.push(arr2);
                }
            ...
            console.log(`[${actionID}] worker return 3, time=`, m4 - m3);
            sendMessage(polygons);
            resolve(true);
        } catch (error) {
            const m4 = new Date().getTime();
            console.error(`[${actionID}] worker error 3, time=`, m4 - m3, error);
            resolve(error);
        }
        // return polygons;
    });
