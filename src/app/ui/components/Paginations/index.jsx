    ...
    return (
        <Paginations.TablePagination
            ...
            pageRecordsRenderer={({ totalRecords, from, to }) => {
                if (totalRecords > 0) {
                    return i18n._('key-unused-Records: {{from}} - {{to}} / {{total}}', {
                        from,
                        to,
                        total: totalRecords
                    });
                ...
                return i18n._('key-unused-Records: {{total}}', { total: totalRecords });
            }}
            pageLengthRenderer={({ pageLength }) => (
                <span>
                    {i18n._('key-unused-{{pageLength}} per page', { pageLength })}
                    <i className="caret" />
                </span>
            )}
        />
