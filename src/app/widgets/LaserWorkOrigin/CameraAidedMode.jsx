...
class CameraAidedMode extends PureComponent {
    static propTypes = {
    ...
    render() {
        const { isIdle, isConnected, workOriginDefined, previewPower, workPower, jogStep, zOffset, zOffsetSlider,
            selectedThickness, customThickness, hoodTouchHeight, hasHoodTouchHeight,
            focusShiftLoaded, zFocus, hasFocusShift, focusShift, modalStage, snapShot, success, actions } = this.props;
        const finalHeight = focusShiftLoaded ? focusShift + selectedThickness : hoodTouchHeight + zFocus - LASER_HOOD_HEIGHT + zOffset;

        const hasSnapShot = !isEmpty(snapShot);
        const snapShotPath = `${DATA_PREFIX}/${snapShot}`;
                ...
                {modalStage === 2 && !focusShiftLoaded && (
                    <div>
                        ...
                        <button
                            ...
                            style={{ display: 'block', width: '100%', marginTop: '5px' }}
                            title={i18n._('Step 4: Set the work origin for running the focal G-Code')}
                        >
                ...
                {modalStage === 3 && (
                    <div>
                        <button
                            type="button"
                            className="sm-btn-large sm-btn-default"
                            onClick={actions.snapShotAndDetect}
                            style={{ display: 'block', width: '100%', marginTop: '5px' }}
                            title={i18n._('Step 7: Detect the thinnest line engraved by the focal G-Code with the camera')}
                        >
                            {i18n._('Detect Optimal Z Offset')}
                        </button>
                        <img
                            src={snapShotPath}
                            role="presentation"
                            style={{ display: hasSnapShot ? 'block' : 'none', width: '100%', marginTop: '10px' }}
                            alt="snapshot"
                        />
                        {success && (
                            <p>
                                {i18n._('Detection succeeded.')}
                            </p>
                        )}
                        {success !== null && !success && (
                            <div>
                                <p>
                                    {i18n._('Detection failed. Please set Z Offset manually.')}
                                </p>
                                <img
                ...
                {modalStage === 4 && (
                    <div>
                        ...
                        <button
                            type="button"
                            className="sm-btn-large sm-btn-default"
                            onClick={actions.confirmWorkOriginCameraAided}
                            disabled={!isIdle || !isConnected || (!focusShiftLoaded && !hasHoodTouchHeight)}
                            style={{ display: 'block', width: '100%', marginTop: '5px' }}
                            title={i18n._('Step 8: Confirm the work origin and home z. Focus shift required')}
                        >
...
export default CameraAidedMode;
