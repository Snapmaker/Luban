...
function recalculateDimensions(root, elem) {
    ...
    let operation = 0;
    // [M]
    if (n <= 2 && transformList.getItem(0).type === 2) {
        operation = 2;
    ...
    if (operation === 2 || operation === 3) {
        ...
        if (transformList.numberOfItems >= 1 && transformList.getItem(0).type === 4) {
            const item = transformList.getItem(0);
            const bBox = getBBox(elem);
            const center = {
            ...
            if (operation === 2) {
                ...
                rotate.setRotate(item.angle, center.x, center.y);
                transformList.replaceItem(rotate, 0);
            }
            if (operation === 3) {
                const newCenter = transformPoint(center, item.matrix);
                const transform = root.createSVGTransform();
                transform.setTranslate(newCenter.x - center.x, newCenter.y - center.y);
                remapElement(elem, transform.matrix);

                const rotate = root.createSVGTransform();
                rotate.setRotate(item.angle, newCenter.x, newCenter.y);
