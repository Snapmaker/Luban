import { NS, getReverseNS } from './namespaces';
import { convertPath, fixEnd } from './path';
import { getHref, setHref, getUrlFromAttr } from './utilities';
...
Object.entries(svgWhiteList_).forEach(([elt, attrs]) => {
    const attNS = {};
    Object.values(attrs).forEach((attr) => {
        if (attr.includes(':')) {
            ...
            attNS[attr] = attr === 'xmlns' ? NS.XMLNS : null;
        }
    });
...
function fixPath(doc) {
    const paths = doc.getElementsByTagNameNS(NS.SVG, 'path');

    for (const path of paths) {
        path.setAttribute('d', convertPath(path));
        fixEnd(path);
    }
}
