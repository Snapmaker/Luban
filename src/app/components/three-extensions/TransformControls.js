...
const TransformControls = function (camera, domElement, boundingBox) {
    ...
    // add by Walker
    const OBJECT_MIN_SIZE = new THREE.Vector3(5, 5, 5);
    this.enabled = true;
    this.uniformScale = true;
    this.translateInParent = true;
    this.bbox = boundingBox;
    let objectSizeStart = new THREE.Vector3();
    var uniformScaleMinValue = 1;
    ...
    this.pointerDown = function (pointer) {
        ...
        if ((pointer.button === 0 || pointer.button === undefined) && this.axis !== null) {
            ...
            if (planeIntersect) {
                ...
                const scaleFactorV3 = OBJECT_MIN_SIZE.clone().divide(objectSizeStart);
                uniformScaleMinValue = Math.min(scaleFactorV3.x, scaleFactorV3.y, scaleFactorV3.z);
            }
    ...
    this.pointerMove = function (pointer) {
        ...
        if (mode === 'translate') {
            ...
            if (object.position.x > this.bbox.max.x) {
                object.position.x = this.bbox.max.x;
            }
            if (object.position.x < this.bbox.min.x) {
                object.position.x = this.bbox.min.x;
            }
            if (object.position.z > this.bbox.max.z) {
                object.position.z = this.bbox.max.z;
            }
            if (object.position.z < this.bbox.min.z) {
                object.position.z = this.bbox.min.z;
            }
        } else if (mode === 'scale') {
            if (axis.search('XYZ') !== -1) {
                ...
                if (scope.uniformScale) {
                    var scaleFactor = 1;
                    // only allow drag X/Y/Z axis
                    if (axis === 'X') {
                        scaleFactor = (pointEnd.x / pointStart.x);
                    }
                    if (axis === 'Y') {
                        scaleFactor = (pointEnd.y / pointStart.y);
                    }
                    if (axis === 'Z') {
                        scaleFactor = (pointEnd.z / pointStart.z);
                    }
                    // set limit: the object size must be larger than (10, 10, 10);
                    scaleFactor = scaleFactor < uniformScaleMinValue ? uniformScaleMinValue : scaleFactor;
                    _tempVector.set(scaleFactor, scaleFactor, scaleFactor);
                } else {
                    if (axis.search('X') === -1) {
                        _tempVector.x = 1;
                    }
...
const TransformControlsGizmo = function () {
    ...
    matRed.color.set(0xe93100);

    var matGreen = gizmoMaterial.clone();
    matGreen.color.set(0x22ac38);

    var matBlue = gizmoMaterial.clone();
    matBlue.color.set(0x00b7ee);
    ...
    matLineRed.color.set(0xe93100);

    var matLineGreen = gizmoLineMaterial.clone();
    matLineGreen.color.set(0x22ac38);

    var matLineBlue = gizmoLineMaterial.clone();
    matLineBlue.color.set(0x00b7ee);
    ...
    var gizmoTranslate = {
        ...
        Y: [
            [new THREE.Mesh(arrowGeometry, matBlue), [0, 1, 0], null, null, 'fwd'],
            [new THREE.Mesh(arrowGeometry, matBlue), [0, 1, 0], [Math.PI, 0, 0], null, 'bwd'],
            [new THREE.Line(lineGeometry, matLineBlue), null, [0, 0, Math.PI / 2]]
        ],
        Z: [
            [new THREE.Mesh(arrowGeometry, matGreen), [0, 0, 1], [Math.PI / 2, 0, 0], null, 'fwd'],
            [new THREE.Mesh(arrowGeometry, matGreen), [0, 0, 1], [-Math.PI / 2, 0, 0], null, 'bwd'],
            [new THREE.Line(lineGeometry, matLineGreen), null, [0, -Math.PI / 2, 0]]
        ]
    };
    ...
    var gizmoRotate = {
        ...
        Y: [
            [new THREE.Line(CircleGeometry(1, 0.5), matLineBlue), null, [0, 0, -Math.PI / 2]],
            [new THREE.Mesh(new THREE.OctahedronBufferGeometry(0.04, 0), matBlue), [0, 0, 0.99], null, [3, 1, 1]],
        ],
        Z: [
            [new THREE.Line(CircleGeometry(1, 0.5), matLineGreen), null, [0, Math.PI / 2, 0]],
            [new THREE.Mesh(new THREE.OctahedronBufferGeometry(0.04, 0), matGreen), [0.99, 0, 0], null, [1, 3, 1]],
        ],
        E: [
            [new THREE.Line(CircleGeometry(1.25, 1), matLineYellowTransparent), null, [0, Math.PI / 2, 0]],
            [new THREE.Mesh(new THREE.CylinderBufferGeometry(0.03, 0, 0.15, 4, 1, false), matLineYellowTransparent), [1.17, 0, 0], [0, 0, -Math.PI / 2], [1, 1, 0.001]],
    ...
    var gizmoScale = {
        ...
        Y: [
            [new THREE.Mesh(scaleHandleGeometry, matBlue), [0, 0.8, 0]],
            [new THREE.Line(lineGeometry, matLineBlue), null, [0, 0, Math.PI / 2], [0.8, 1, 1]]
        ],
        Z: [
            [new THREE.Mesh(scaleHandleGeometry, matGreen), [0, 0, 0.8], [Math.PI / 2, 0, 0]],
            [new THREE.Line(lineGeometry, matLineGreen), null, [0, -Math.PI / 2, 0], [0.8, 1, 1]]
        ]
    };
