...
export const xformPoint = (t, p) => {
    const p0 = p[0];
    p[0] = t[0] * p0 + t[2] * p[1] + t[4];
    p[1] = t[1] * p0 + t[3] * p[1] + t[5];
};

export function xformMultiply(t, s) {
    const t0 = t[0];
    const t1 = t[1];
    const t2 = t[2];
    const t3 = t[3];
    ...
    t[2] = t0 * s[2] + t2 * s[3];
    t[3] = t1 * s[2] + t3 * s[3];
    t[4] = t0 * s[4] + t2 * s[5] + t[4];
    t[5] = t1 * s[4] + t3 * s[5] + t[5];
}
...
export const cssColor2Hex = (colorName) => {
    if (CSS_COLORS[colorName]) {
        return CSS_COLORS[colorName];
    }
    return null;
};
