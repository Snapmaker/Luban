...
class GridLine {
    ...
    constructor(offsetX, sizeX, stepX, sizeY, stepY, colorCenterLine, colorGrid) {
        ...
        for (let x = offsetX; x <= offsetX + sizeX; x += stepX) {
            const geometry = new THREE.Geometry();
            const material = new THREE.LineBasicMaterial({
                vertexColors: THREE.VertexColors
            });
            // const color = (x === 0) ? colorCenterLine : colorGrid;
            const color = (x === offsetX) ? colorCenterLine : colorGrid;

            geometry.vertices.push(
                // new THREE.Vector3(x, -sizeY, 0),
                new THREE.Vector3(x, 0, 0),
                new THREE.Vector3(x, sizeY, 0),
                // new THREE.Vector3(offsetX + x, 0, 0),
                // new THREE.Vector3(offsetX + x, sizeY, 0),
            );
        ...
        for (let y = 0; y <= sizeY; y += stepY) {
            ...
            geometry.vertices.push(
                ...
                new THREE.Vector3(offsetX + sizeX, y, 0),
            );
            geometry.colors.push(color, color);
